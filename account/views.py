...
def account_edit(request, response_format='html'):
    ...
    return render_to_response('account/account_edit',
                              {'profile': profile, 'form': Markup(form.as_ul())},
                              context_instance=RequestContext(request), response_format=response_format)
...
def settings_view(request, response_format='html'):
    ...
    # default permissions
    try:
        conf = ModuleSetting.get_for_module('treeio.core', 'default_permissions', user=user)[0]
    ...
    # default perspective
    try:
        conf = ModuleSetting.get_for_module('treeio.core', 'default_perspective', user=user)[0]
        ...
        if 'massform' in request.POST:
            for key in request.POST:
                if 'mass-setting' in key:
                    try:
                        setting = NotificationSetting.objects.get(pk=request.POST[key])
                        form = NotificationSettingMassActionForm(request.POST, instance=setting)
