        ...
        for module in modules:
            if request.user.get_profile().has_permission(module):
                try:
                    ...
                    imodule = __import__(import_name, fromlist=[str(module.name)])
        ...
        if renderable_events:
            ...
            for event in renderable_events:
                if event.is_renderable(date, hour):
                    ...
                    event.margin_step[str(date)] = current_margin = current_margin + 1
                    if event.is_full_day(date):
                        height = (self.end_hour - self.start_hour + 1) * 40 - 5
                    else:
                        duration = event.get_duration(date, self.start_hour, self.end_hour)
                    ...
                    style = "width: %.2f%%; height: %dpx; margin-left: %.2f%%" % (width, height, margin)
                    output += event.render_for_datehour(date, hour, css_class, style)
