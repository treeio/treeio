...
## Assets, Equities
    ...
    if 'purchase_date_from' in args and args['purchase_date_from']:
        purchase_date_from = datetime.date(datetime.strptime(args['purchase_date_from'], '%m/%d/%Y'))
        query = query & Q(purchase_date__gte=purchase_date_from)

    if 'purchase_date_to' in args and args['purchase_date_to']:
        purchase_date_to = datetime.date(datetime.strptime(args['purchase_date_to'], '%m/%d/%Y'))
...
## Liabilities, Receivables
    ...
    if 'due_date_from' in args and args['due_date_from']:
        due_date_from = datetime.date(datetime.strptime(args['due_date_from'], '%m/%d/%Y'))
        query = query & Q(due_date__gte=due_date_from)

    if 'due_date_to' in args and args['due_date_to']:
        due_date_to = datetime.date(datetime.strptime(args['due_date_to'], '%m/%d/%Y'))
...
def category_edit(request, category_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = CategoryForm(request.user.get_profile(), request.POST, instance=category)
            if form.is_valid():
                category = form.save()
        ...
        for key in request.POST:
            if 'mass-transaction' in key:
                try:
                    ...
                    form = MassActionForm(request.user.get_profile(), request.POST, instance=transaction)
        ...
        for key in request.POST:
            if 'mass-liability' in key:
                try:
                    ...
                    form = MassActionForm(request.user.get_profile(), request.POST, instance=liability)
...
def account_edit(request, account_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = AccountForm(request.user.get_profile(), request.POST, instance=account)
            if form.is_valid():
...
def index_assets(request, response_format='html'):
    ...
    assets = Object.filter_by_request(request, Asset.objects.filter(query), mode="r")
...
def asset_edit(request, asset_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = AssetForm(request.user.get_profile(), request.POST, instance=asset)
            if form.is_valid():
                asset = form.save()
...
def index_equities(request, response_format='html'):
    ...
    filters = EquityFilterForm(request.user.get_profile(), 'title', request.GET)
    
    equities = Object.filter_by_request(request, Equity.objects.filter(query), mode="r")
...
def equity_edit(request, equity_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = EquityForm(request.user.get_profile(), request.POST, instance=equity)
            if form.is_valid():
                equity = form.save()
...
def transaction_add(request, liability_id=None, order_id=None, response_format='html'):
    ...
    transactions = Object.filter_by_request(request, Transaction.objects, mode="r")
    if request.POST:
        if not 'cancel' in request.POST:
            transaction = Transaction()
            form = TransactionForm(request.user.get_profile(), None, None, request.POST, instance=transaction)
            if form.is_valid():
        ...
        form = TransactionForm(request.user.get_profile(), liability_id=liability_id, order_id=order_id)
...
def transaction_edit(request, transaction_id, response_format='html'):
    ...
    if request.POST:
        ...
        form = TransactionForm(request.user.get_profile(), None, None, instance=transaction)
...
def index_liabilities(request, response_format='html'):
    ...
    filters = LiabilityFilterForm(request.user.get_profile(), 'title', request.GET)
    
    liabilities = Object.filter_by_request(request, Liability.objects.filter(query), mode="r")
...
def liability_edit(request, liability_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = LiabilityForm(request.user.get_profile(), request.POST, instance=liability)
            if form.is_valid():
...
def index_receivables(request, response_format='html'):
    ...
    receivables = Object.filter_by_request(request, Liability.objects.filter(query), mode="r")
...
def receivable_add(request, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            ...
            form = ReceivableForm(request.user.get_profile(), request.POST, instance=receivable)
            if form.is_valid():
...
def balance_sheet(request, response_format='html'):
    ...
    if my_company:
        ...
        for product in Product.objects.filter(active=True, buy_price__isnull=False, stock_quantity__gt=0, 
                                              trash=False):
            balance['assets_inventories'] += product.buy_price * product.stock_quantity
        ...
        for liability in Liability.objects.filter(source=my_company, trash=False):
            ...
            paid = liability.transaction_set.filter(source=my_company, trash=False).aggregate(Sum('value'))
            if paid['value__sum']:
        ...
        balance['equity_liabilities_total'] = balance['equity_total'] + balance['liabilities_total']
...
def index_accounts(request, response_format='html'):
    ...
    filters = AccountFilterForm(request.user.get_profile(), 'title', request.GET)
    
    all_accounts = Object.filter_by_request(request, Account.objects.filter(query))
...
def settings_view(request, response_format='html'):
    ...
    categories = Object.filter_by_request(request, Category.objects.filter(trash=False))
    
    # all currencies
    currencies = Object.filter_by_request(request, Currency.objects.filter(trash=False))
...
def currency_add(request, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            currency = Currency()
            form = CurrencyForm(request.user.get_profile(), request.POST, instance=currency)
            if form.is_valid():
...
def tax_add(request, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            tax = Tax()
            form = TaxForm(request.user.get_profile(), request.POST, instance=tax)
            if form.is_valid():
                tax = form.save()
