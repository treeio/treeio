...
class Currency(Object):
    ...
    factor = models.DecimalField(_('factor'), max_digits=10, decimal_places=4,
        help_text=_('Specifies the ratio to the base currency, e.g. 1.324'),
        default = 1)
...
class Asset(Object):
    ...
    asset_type = models.CharField(max_length=32,
                                  choices=(('fixed', 'Fixed'), ('intangible', 'Intangible')),
                                  default='fixed')
    initial_value = models.DecimalField(max_digits=20, decimal_places=2, default=0)
    lifetime = models.DecimalField(max_digits=20, decimal_places=0, blank=True, null=True)
    endlife_value = models.DecimalField(max_digits=20, decimal_places=2, blank=True, null=True)
    depreciation_rate = models.DecimalField(max_digits=20, decimal_places=5, blank=True, null=True)
    depreciation_type = models.CharField(max_length=32, blank=True, null=True,
                                         choices=(('straight', 'Straight Line'), 
                                                  ('reducing', 'Reducing balance')),
                                         default='straight')
    purchase_date = models.DateField(blank=True, null=True, default=datetime.now)
...
class Account(Object):
    ...
    balance_display = models.DecimalField(max_digits=20, decimal_places=2, default=0)
    details = models.TextField(blank=True, null=True)
...
class Liability(Object):
    ...
    name = models.CharField(max_length=512)
    source = models.ForeignKey(Contact, related_name='finance_liability_source')
    target = models.ForeignKey(Contact, related_name='finance_liability_target')
...
class Transaction(Object):
    """ Transaction model """
    name = models.CharField(max_length=512)
    source = models.ForeignKey(Contact, related_name='finance_transaction_source')
    target = models.ForeignKey(Contact, related_name='finance_transaction_target')
    liability = models.ForeignKey(Liability, blank=True, null=True, on_delete=models.SET_NULL)
    category = models.ForeignKey(Category, blank=True, null=True, on_delete=models.SET_NULL)
    account = models.ForeignKey(Account)
