...
class MassActionForm(forms.Form):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields['category'].queryset = Object.filter_permitted(user, Category.objects, mode='x')
...
class AccountForm(forms.ModelForm):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields['balance_currency'].widget.attrs.update({'popuplink': reverse('finance_currency_add')})
        try:
            self.fields['balance_currency'].initial = Currency.objects.get(is_default=True)
        except:
            pass
    ...
    class Meta:
        "Account Form"
        model = Account
        fields = ('name', 'owner', 'balance_currency', 'balance_display', 'details')
    ...
    def __init__(self, user, skip=[], *args, **kwargs):
        ...
        if 'purchase_date_from' in skip:
            ...
            self.fields['purchase_date_from'] = forms.DateField(label="Purchase Date From:", 
                                                                required = False)
            self.fields['purchase_date_from'].widget.attrs.update({'class': 'datepicker'})
            self.fields['purchase_date_from'].label = _("Purchase Date From")
            
        if 'purchase_date_to' in skip:
            del self.fields['purchase_date_to']
        else:
            self.fields['purchase_date_to'] = forms.DateField(label="Purchase Date To:", required = False)
            self.fields['purchase_date_to'].widget.attrs.update({'class': 'datepicker'})
...
class ReceivableForm(forms.ModelForm):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields['source'].widget.attrs.update({'class': 'autocomplete', 
                                                   'callback': reverse('identities_ajax_contact_lookup')})
        self.fields['source'].widget.attrs.update({'popuplink': reverse('identities_contact_add')})
    ...
    class Meta:
        "Receivable Form"
        model = Liability
        fields = ('name', 'category', 'source', 'target', 'account', 'due_date', 'value_currency', 'value_display', 'details')
...
class TransactionForm(forms.ModelForm):
    ...
    def __init__(self, user, liability_id=None, order_id=None, *args, **kwargs):
        ...
        self.fields['liability'].queryset = Object.filter_permitted(user, Liability.objects)
        ...
        if order_id:
            ...
            try:
                conf = ModuleSetting.get_for_module('treeio.finance', 'my_company')[0]
                self.fields['target'].initial = Contact.objects.get(pk=long(conf.value))
...
class SettingsForm(forms.Form):
    ...
    default_currency = forms.ModelChoiceField(label='Base Currency', queryset=[])
    my_company = forms.ModelChoiceField(label='My Company', queryset=[])
    default_account = forms.ModelChoiceField(label='Default Account', queryset=[])
...
class CurrencyForm(forms.ModelForm):
    ...
    code = forms.ChoiceField(label=_("Currency Code"), choices=standard_currencies)
