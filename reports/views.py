            ...
            for key in request.POST:
                if 'mass-report' in key:
                    try:
                        ...
                        form = MassActionForm(request.user.get_profile(), request.POST, instance=report)
...
def _get_objects():
    object_types = Object.objects.all().values('object_type').distinct().order_by('object_type')
    #TODO: filter these
    object_names = ["(%s)  %s" %
...
def chart_add(request, report_id=None, response_format='html'):
    ...
    if request.POST:
        form = ChartForm(request.user.get_profile(), request.POST, report_id=report_id)
        if form.is_valid():
            chart = form.save()
...
def chart_edit(request, chart_id=None, response_format='html'):
    ...
    if request.POST:
        form = ChartForm(request.user.get_profile(), request.POST, chart_id=chart_id)
        if form.is_valid():
            chart = form.save()
            chart.set_user_from_request(request)
...
def index_owned(request, response_format='html'):
    ...
    reports = Object.filter_by_request(request, Report.objects.filter(creator=request.user.get_profile()))
...
def report_filter(request, report_id, field_name, response_format='html'):
    ...
    if request.POST:
        FilterForm(request.user.get_profile(), request.POST, report=report, field_name=field_name).save()
        return HttpResponseRedirect(reverse('reports_report_edit', args=[report.id]))
    
    else:
        form = FilterForm(request.user.get_profile(), report=report, field_name=field_name)
