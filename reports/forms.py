...
class SetForm(forms.Form):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        for i, name in enumerate(names):
            self.fields[names[i]] = forms.BooleanField(label=_(names[i].title().replace('_', ' ')), required=False)
            self.fields['obj'] = forms.CharField(initial=str(obj), widget=forms.widgets.HiddenInput())
...
class ChartForm(forms.Form):
    ...
    def clean_grouping(self):
        ...
        if not data:
            raise forms.ValidationError(_("Please enter a field to group your data by."))
        return data
