...
class ItemForm(forms.Form):
    ...
    def _get_form_field(self, field, value=None):
        ...
        if field.field_type == 'text':
            ...
            form_field = forms.URLField(label=field.label, widget=forms.TextInput(attrs={'size':'50'}))
        elif field.field_type == 'picture':
            form_field = forms.ImageField(label=field.label, widget=forms.FileInput)
        ...
        if value:
            if isinstance(form_field, forms.FileField) and value.value:
                form_field = forms.ChoiceField(label=field.label, widget=forms.RadioSelect())
    ...
    def __init__(self, user, item_type, *args, **kwargs):
        ...
        self.fields['parent']       = forms.ModelChoiceField(label=_('Parent'), queryset=[], required=False)
        self.fields['status']       = forms.ModelChoiceField(label=_('Status'), queryset=[], required=True)
        self.fields['manufacturer'] = forms.ModelChoiceField(label=_('Manufacturer'), 
                                                             queryset=[], required=False)
        self.fields['supplier']     = forms.ModelChoiceField(label=_('Supplier'), queryset=[], required=False)
        self.fields['owner']        = forms.ModelChoiceField(label=_('Owner'), queryset=[], required=False)
        self.fields['location']     = forms.ModelChoiceField(label=_('Location'), queryset=[], required=False)
        self.fields['asset']        = forms.ModelChoiceField(label=_('Asset Record'), 
                                                             queryset=[], required=False)
        
        self.fields['parent'].queryset       = Object.filter_permitted(user, Item.objects, mode='x')
        self.fields['status'].queryset       = Object.filter_permitted(user, ItemStatus.objects, mode='x')
        self.fields['manufacturer'].queryset = Object.filter_permitted(user, Contact.objects, mode='x')
        self.fields['supplier'].queryset     = Object.filter_permitted(user, Contact.objects, mode='x')
        self.fields['owner'].queryset        = Object.filter_permitted(user, Contact.objects, mode='x')
        self.fields['location'].queryset     = Object.filter_permitted(user, Location.objects, mode='x')
        self.fields['asset'].queryset        = Object.filter_permitted(user, Asset.objects, mode='x')
        
        self.fields['manufacturer'].widget.attrs.update({'class': 'autocomplete', 
                                                'callback': reverse('identities_ajax_contact_lookup')})
        self.fields['manufacturer'].widget.attrs.update({'popuplink': reverse('identities_contact_add')})
        ...
        self.fields['location'].widget.attrs.update({'popuplink': reverse('infrastructure_location_add')})
        self.fields['location'].widget.attrs.update({'class': 'autocomplete', 
                                                  'callback': reverse('identities_ajax_location_lookup')})
        self.fields['asset'].widget.attrs.update({'popuplink': reverse('finance_asset_add')})
            ...
            if self.instance:
                ...
                for value in values:
                    if value.field == field:
                        ...
                        self.fields[field_name] = self._get_form_field(field, value)
...
class ServiceRecordForm(forms.ModelForm):
    ...
    def __init__(self, user, service_record=None, *args, **kwargs):
        ...
        self.fields['items'].queryset = Object.filter_permitted(user, Item.objects.all(), mode='x')
        self.fields['items'].help_text = ""
        self.fields['supplier'].queryset = Object.filter_permitted(user, Contact.objects.all(), mode='x')
        self.fields['payments'].queryset = Object.filter_permitted(user, Transaction.objects.all(), mode='x')
        self.fields['payments'].help_text = ""
        self.fields['payments'].widget.attrs.update({'popuplink': reverse('finance_transaction_add')})
    ...
    class Meta:
        "Service Record Form"
        model = ItemServicing
        fields = ('name', 'items', 'supplier', 'start_date', 'expiry_date', 'payments', 'details')
...
class FilterForm(forms.ModelForm):
    ...
    class Meta:
        "Filter Form"
        model = Item
        fields = ('item_type', 'status', 'manufacturer', 'supplier', 'owner', 'location')
...
class SettingsForm(forms.Form):
    ...
    default_item_status = forms.ModelChoiceField(label='Default Item Status', queryset=[])
