            ...
            for key in request.POST:
                if 'mass-item' in key:
                    try:
                        ...
                        form = MassActionForm(request.user.get_profile(), request.POST, instance=item)
...
def index_owned(request, response_format='html'):
    ...
    if request.GET:
        query = query & _get_filter_query(request.GET)
    items = Object.filter_by_request(request, Item.objects.filter(query).order_by('-date_created'))
...
def type_view(request, type_id, response_format='html'):
    ...
    filters = FilterForm(request.user.get_profile(), ['item_type'], request.GET)
...
def location_edit(request, location_id, response_format='html'):
    ...
    if request.POST:
        ...
        form = LocationForm(request.user.get_profile(), None, instance=location)
...
def service_record_index(request, response_format='html'):
    ...
    if request.GET:
        query = query & _get_filter_query(request.GET)
    service_records = Object.filter_by_request(request, ItemServicing.objects.filter(query))
