            ...
            for key in request.POST:
                if 'mass-changeset' in key:
                    try:
                        ...
                        form = MassActionForm(request.user.get_profile(), request.POST, instance=changeset)
...
def index_resolved(request, response_format='html'):
    ...
    if request.GET:
        query = query & _get_filter_query(request.GET)
        filters = FilterForm(request.user.get_profile(), 'resolved_by', request.GET)
    else:
...
def set_edit(request, set_id, response_format='html'):
    ...
    if request.POST:
        form = ChangeSetForm(request.user.get_profile(), request.POST, instance=changeset)
        if form.is_valid():
            changeset = form.save()
