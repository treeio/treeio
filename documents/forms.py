...
class FolderForm(ModelForm):
    ...
    def __init__(self, user, folder_id, *args, **kwargs):
        ...
        self.fields['parent'].queryset = Object.filter_permitted(user, Folder.objects, mode='x')
        if folder_id:
...
class DocumentForm(ModelForm):
    ...
    def __init__(self, user, folder_id, *args, **kwargs):
        ...
        self.fields['folder'].widget.attrs.update({'popuplink': reverse('documents_folder_add')})
        if folder_id:
            self.fields['folder'].initial = folder_id
        else:
            try:
                self.fields['folder'].initial = self.fields['folder'].queryset[0].id
            except:
                pass
