...
class EmailReplier(EmailReceiver):
    subject_regex = re.compile("(Re:\s*)*\[Tree.io \#(?P<id>\d+)\] .+: .+ '.+' - .+", re.IGNORECASE | re.MULTILINE)
    ...
    def process_msg(self, msg, attrs, attachments):
        ...
        #check subject and get Object, subject should be fit subject's regex in Object.notify_subscribers
