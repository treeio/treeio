...
def render_to_ajax(template_name, context={}, context_instance=None):
    ...
    content = render_to_string(template_name, context, context_instance, response_format)
                    ...
                    try:
                        ...
                        if update.author:
                            if update.record_type == 'manual' or update.record_type == 'share':
                                try:
                                    message['image'] = update.author.get_contact().get_picture()
                                except:
                                    pass 
                            message['title'] = unicode(update.author)
                        for obj in update.about.all():
                            message['message'] = "(%s) %s:<br />%s" % (obj.get_human_type(), unicode(obj), message['message'])
