...
PERMISSION_CHOICES = (
    ...
    ('userallgroupsreadonly', 'Automatic, User and All Their Groups. READ ONLY'),
    ...
    ('nomoduleusergroupreadonly', 'Automatic, Skip Module, User and Default Group. READ ONLY'),
    ('nomoduleuserallgroups', 'Automatic, Skip Module, User and All Their Groups'),
    ('nomoduleuserallgroupsreadonly', 'Automatic, Skip Module, User and All Their Groups. READ ONLY'),
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        if getattr(settings, 'HARDTREE_SUBSCRIPTION_CUSTOMIZATION', True):
            ...
            if logopath:
                ...
                form_field = forms.ChoiceField(label=_("Logo"), widget=forms.RadioSelect())
...
class UserForm(forms.ModelForm):
    """ User form """
    perspective = ModelChoiceField(label='Perspective', queryset=[], required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance:
            try:
                self.fields['perspective'].initial = self.instance.get_perspective()
            except:
                pass
        ...
        if self.instance and not self.instance.id:
            ...
            if user_limit > 0:
                ...
                if user_number >= user_limit:
                    raise forms.ValidationError(_("Sorry, but your subscription does not allow more than %d users. You're currently at your limit.") % (user_limit))
        return data
...
class GroupForm(forms.ModelForm):
    """ Group form """
    perspective = ModelChoiceField(label=_('Perspective'), queryset=[], required=False)
