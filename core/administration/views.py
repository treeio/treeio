...
def index_perspectives(request, response_format='html'):
    ...
    filters = FilterForm(request.user.get_profile(), 'perspective', request.GET)
...
def perspective_edit(request, perspective_id, response_format='html'):
    ...
    other_perspectives = Perspective.objects.filter(trash=False).exclude(id=perspective_id)
    ...
    if request.POST:
        ...
        form = PerspectiveForm(request.user.get_profile(), instance=perspective)
...
def perspective_delete(request, perspective_id, response_format='html'):
    ...
    if not other_perspectives:
        ...
        message = _("This is your only Perspective with Administration module. You would be locked out!")
    else:
        if request.POST:
...
def user_view(request, user_id, response_format='html'):
    ...
    return render_to_response('core/administration/user_view',
                              {'profile': user, 'contacts': contacts, 'modules': modules},
                              context_instance=RequestContext(request), response_format=response_format)
...
def contact_setup(request, response_format='html'):
    ...
    if person_type and request.POST:
        contact_form = ContactSetupForm(person_type, instance=contact, data=request.POST, files=request.FILES, prefix='person')
        company_form = ContactSetupForm(company_type, instance=company, data=request.POST, files=request.FILES, prefix='company')
        ...
        contact_form = ContactSetupForm(person_type, instance=contact, prefix='person') if person_type else None
        company_form = ContactSetupForm(company_type, instance=company, prefix='company') if company_type else None
...
def settings_view(request, response_format='html'):
    ...
    try:
        conf = ModuleSetting.get_for_module('treeio.core', 'default_timezone')[0]
...
def settings_edit(request, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = SettingsForm(request.user.get_profile(), request.POST, request.FILES)
            if form.is_valid():
                form.save()
