...
def DatabaseCreation(domain):
    ...
    try:
        BaseDatabaseCreation = import_module('.creation', connection.settings_dict['ENGINE']).DatabaseCreation
    except:
        BaseDatabaseCreation = import_module('.creation', 'django.db.backends').BaseDatabaseCreation

    class DBCreation(BaseDatabaseCreation):

        def __init__(self, connection):
            super(DBCreation, self).__init__(connection)
            self.database_name = self.connection.settings_dict.get('NAME') or super(DBCreation, self)._get_test_db_name()
