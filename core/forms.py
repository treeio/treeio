...
class SubscribeForm(forms.Form):
    ...
    def __init__(self, instance, *args, **kwargs):
        ...
        self.fields['subscriber'].queryset = User.objects.exclude(pk__in=subscriptions)
    ...
    def __init__(self, user, response_format, instance, *args, **kwargs):
        ...
        if not 'ajax' in response_format:
            ...
            for obj in queryset:
                ...
                name = do_truncate(do_striptags(unicode(obj.object_name)), 20, True)
                if human_type:
                    name += u" (" + human_type + u")"
...
class PasswordResetForm(forms.Form):
    ...
    def clean_username(self):
        ...
        if len(users) == 0:
            raise forms.ValidationError(_("Sorry, we don't know that user or e-mail."))
        else:
            username = users[0]
        return username
...
class InvitationForm(forms.Form):
    ...
    def __init__(self, invitation=None, *args, **kwargs):
        ...
        self.fields['username']       = forms.CharField(max_length=255, label=_("Username"))
        self.fields['name']           = forms.CharField(max_length=255, label=_("Your name"))
...
class LocationForm(forms.ModelForm):
    ...
    def __init__(self, user, location_id, *args, **kwargs):
        ...
        self.fields['parent'].queryset = Object.filter_permitted(user, Location.objects, mode='x')
        if location_id:
...
class SqlSettingsForm(forms.Form):
    ...
    sql_password = forms.CharField(max_length=128, required=False, widget=forms.PasswordInput)
