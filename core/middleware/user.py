...
class CommonMiddleware():
    ...
    def process_request(self, request):
        ...
        if hasattr(request, 'user') and request.user.is_authenticated():
            ...
            do_fresh_subscribers = curry(self.do_fresh_subscribers, user, request)
            send_notifications_on_save = curry(self.send_notifications_on_save, user, request)
            send_notifications_on_delete = curry(self.send_notifications_on_delete, user, request)
            send_notifications_on_m2m = curry(self.send_notifcations_on_m2m, user, request)
            
            signals.pre_save.connect(send_notifications_on_save, dispatch_uid=request, weak=False)
            signals.post_save.connect(do_fresh_subscribers, dispatch_uid=request, weak=False)
            signals.pre_delete.connect(send_notifications_on_delete, dispatch_uid=request, weak=False)
            signals.m2m_changed.connect(send_notifications_on_m2m, dispatch_uid=request, weak=False)
    ...
    def send_notifcations_on_m2m(self, user, request, sender, instance, action, reverse, model, pk_set, **kwargs):
        ...
        if isinstance(instance, Object):
            ...
            if action == "pre_clear" or action == "pre_remove":
                ...
                self.objects[unicode(user.id)].update({unicode(instance.id): original})
                ...
                if unicode(user.id) in self.objects and unicode(instance.id) in self.objects[unicode(user.id)]:
                    original = self.objects[unicode(user.id)][unicode(instance.id)]
                    if not original == updated:
                        try:
                            instance.create_notification('m2m', user, field=attr, original=original, updated=updated)
                        except:
                            pass
            ...
            if action == "post_add" and attr == 'about':
                ...
                subscribers = set()
                if instance.author: # Send notifications to author Contact's subscribers too
        ...
        if field.name not in getattr(settings, 'HARDTREE_TIMEZONE_BLACKLIST', []):
            ...
            isinstance(field, models.DateField):
                if getattr(instance, field.name):
                    ...
                    if sign == "-":
                        new_date = cur_date + timedelta(hours=hours, minutes=mins)
                    else: 
                        new_date = cur_date - timedelta(hours=hours, minutes=mins)
...
class AuthMiddleware(object):
    ...
    def process_request(self, request):
        ...
        if user:
            ...
            del params['authkey']
            url = urlparse.urlunparse((p.scheme, p.netloc, p.path, p.params, urllib.urlencode(params, doseq=True), p.fragment))
