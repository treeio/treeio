...
def pager(context, items, plength=None):
    ...
    if pagenum > 1:
        if current >= 1:
            pages.append({'page': '&larr;', 'skip': (current-1)*plength, 'mover': True})
        else:
            pages.append({'page': '&larr;', 'skip': 0, 'mover': True})
        if start > 1:
            pages.append({'page': '...', 'skip': (start-1)*plength, 'mover': False})
            maxpages -= 1
        for i in range(start, pagenum+1):
            if i > (maxpages + start + 1):
                pages.append({'page': '...', 'skip': (i)*plength, 'mover': False})
                break
            else:
                pages.append({'page': i, 'skip': (i-1)*plength})
        if current + 1 == pagenum:
            pages.append({'page': '&rarr;', 'skip': current*plength, 'mover': True})
        else:
            pages.append({'page': '&rarr;', 'skip': (current+1)*plength, 'mover': True})
