            ...
            for key in request.POST:
                if 'mass-order' in key:
                    try:
                        ...
                        form = MassActionForm(request.user.get_profile(), request.POST, instance=order)
            ...
            for key in request.POST:
                if 'mass-lead' in key:
                    try:
                        ...
                        form = LeadMassActionForm(request.user.get_profile(), request.POST, instance=lead)
...
def index(request, response_format='html'):
    ...
    orders = Object.filter_by_request(request, SaleOrder.objects.filter(query), mode="r")
...
def index_assigned(request, response_format='html'):
    ...
    orders = Object.filter_by_request(request, SaleOrder.objects.filter(query), mode="r")
    filters = OrderFilterForm(request.user.get_profile(), 'assigned', request.GET)
    statuses = Object.filter_by_request(request, SaleStatus.objects, mode="r")
...
def ordered_product_add(request, order_id=None, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            ...
            form = OrderedProductForm(request.user.get_profile(), order, request.POST, instance=ordered_product)
            if form.is_valid():
...
def ordered_product_edit(request, ordered_product_id, response_format='html'):
    ...
    if request.POST:
        ...
        form = OrderedProductForm(request.user.get_profile(), order, instance=ordered_product)
...
def subscription_add(request, order_id=None, product_id=None, productset_id=None, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = SubscriptionForm(request.user.get_profile(), request.POST, instance=subscription)
            if form.is_valid():
        ...
        form = SubscriptionForm(request.user.get_profile(), instance=subscription)
...
def product_index(request, response_format='html'):
        ...
        for key in request.POST:
            if 'mass-product' in key:
                try:
                    ...
                    form = ProductMassActionForm(request.user.get_profile(), request.POST, instance=product)
    ...
    statuses = Object.filter_by_request(request, SaleStatus.objects, mode="r")
    products = Object.filter_by_request(request, Product.objects.filter(query), mode="r")
...
def product_add(request, parent_id=None, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            product = Product()
            form = ProductForm(request.user.get_profile(), None, request.POST, instance=product)
            if form.is_valid():
                product = form.save()
...
def product_view(request, product_id, response_format='html'):
    ...
    subproducts = Object.filter_by_request(request, Product.objects.filter(parent=product))
...
def lead_index(request, response_format='html'):
    ...
    statuses = Object.filter_by_request(request, SaleStatus.objects, mode="r")
    leads = Object.filter_by_request(request, Lead.objects.filter(query), mode="r")
...
def lead_add(request, lead_id=None, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            lead = Lead()
            form = LeadForm(request.user.get_profile(), request.POST, instance=lead)
            if form.is_valid():
                lead = form.save()
...
def opportunity_index(request, response_format='html'):
    ...
    filters = OpportunityFilterForm(request.user.get_profile(), '', request.GET)

    statuses = Object.filter_by_request(request, SaleStatus.objects, mode="r")
    opportunities = Object.filter_by_request(request, Opportunity.objects.filter(query), mode="r")
...
def opportunity_add(request, lead_id=None, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:    
            form = OpportunityForm(request.user.get_profile(), lead, request.POST)
            if form.is_valid():
...
def opportunity_edit(request, opportunity_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = OpportunityForm(request.user.get_profile(), None, request.POST, instance=opportunity)
            if form.is_valid():
        ...
        form = OpportunityForm(request.user.get_profile(), None, instance=opportunity)
...
def order_add(request, lead_id=None, opportunity_id=None, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            order = SaleOrder()
            form = OrderForm(request.user.get_profile(), lead, opportunity, request.POST, instance=order)
            if form.is_valid():
                order = form.save()
...
def order_edit(request, order_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = OrderForm(request.user.get_profile(), None, None, request.POST, instance=order)
            if form.is_valid():
                order = form.save()
                return HttpResponseRedirect(reverse('sales_order_view', args=[order.id]))
        else:
        ...
        form = OrderForm(request.user.get_profile(), None, None, instance=order)
...
def status_add(request, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            status = SaleStatus()
            form = SaleStatusForm(request.user.get_profile(), request.POST, instance=status)
            if form.is_valid():
                status = form.save()
...
def source_add(request, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            source = SaleSource()
            form = SaleSourceForm(request.user.get_profile(), request.POST, instance=source)
            if form.is_valid():
                source = form.save()
