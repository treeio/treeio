            ...
            for key in request.POST:
                if 'mass-ticket' in key:
                    try:
                        ...
                        form = MassActionForm(request.user.get_profile(), request.POST, instance=ticket)
...
def index(request, response_format='html'):
    ...
    if request.GET:
        ...
        tickets = Object.filter_by_request(request, Ticket.objects.filter(status__hidden=False))
...
def status_edit(request, status_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = TicketStatusForm(request.user.get_profile(), request.POST, instance=status)
            if form.is_valid():
                status = form.save()
...
def queue_edit(request, queue_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = QueueForm(request.user.get_profile(), request.POST, instance=queue)
            if form.is_valid():
                queue = form.save()
...
def ticket_edit(request, ticket_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = TicketForm(request.user.get_profile(), None, agent, request.POST, instance=ticket)
            if form.is_valid():
                ticket = form.save()
                return HttpResponseRedirect(reverse('services_ticket_view', args=[ticket.id]))
        else:
        ...
        form = TicketForm(request.user.get_profile(), None, agent, instance=ticket)
        ...
        if not 'cancel' in request.POST:
            ...
            if not agent:
                ...
                try:
                    ...
                    pass
            form = TicketForm(profile, queue, agent, request.POST, instance=ticket)
            if form.is_valid():
                ticket = form.save()
...
def service_catalogue(request, response_format='html'):
    ...
    services = Object.filter_by_request(request, Service.objects.filter(parent__isnull=True))
...
def service_edit(request, service_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = ServiceForm(request.user.get_profile(), request.POST, instance=service)
            if form.is_valid():
                service = form.save()
...
def sla_edit(request, sla_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = ServiceLevelAgreementForm(request.user.get_profile(), request.POST, instance=sla)
            if form.is_valid():
                sla = form.save()
        ...
        form = ServiceLevelAgreementForm(request.user.get_profile(), instance=sla)
...
def settings_view(request, response_format='html'):
    ...
    if send_email_to_caller:
        ...
        try:
            ticket = Object.filter_by_request(request, Ticket.objects.filter(status__hidden=False, caller__isnull=False))[0]
        except IndexError:
        ...
        if send_email_template:
            try:
                send_email_example = render_string_template(send_email_template, {'ticket': ticket})
            except:
                send_email_example = render_to_string('services/emails/notify_caller', {'ticket': ticket}, response_format='html')
        else:
            send_email_example = render_to_string('services/emails/notify_caller', {'ticket': ticket}, response_format='html')
...
def agent_edit(request, agent_id, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            form = AgentForm(request.user.get_profile(), request.POST, instance=view_agent)
            if form.is_valid():
...
def agent_add(request, response_format='html'):
    ...
    if request.POST:
        if not 'cancel' in request.POST:
            ...
            form = AgentForm(request.user.get_profile(), request.POST, instance=new_agent)
            if form.is_valid():
                new_agent = form.save()
