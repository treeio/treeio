...
class ServiceLevelAgreement(Object):
    ...
    client         = models.ForeignKey(Contact, related_name="client_sla", blank=True, null=True)
...
class TicketQueue(Object):
    ...
    active                  = models.BooleanField(default=True)
    parent                  = models.ForeignKey('self', blank=True, null=True, related_name='child_set')
    default_ticket_status   = models.ForeignKey(TicketStatus, blank=True, null=True, on_delete=models.SET_NULL)
    default_ticket_priority = models.IntegerField(default=3,
                                                  choices=((5, 'Highest'), (4, 'High'), (3, 'Normal'), 
                                                           (2, 'Low'), (1, 'Lowest')))
    default_service         = models.ForeignKey(Service, blank=True, null=True, on_delete=models.SET_NULL)
    waiting_time            = models.PositiveIntegerField(blank=True, null=True)
    next_queue              = models.ForeignKey('self', blank=True, null=True, related_name='previous_set', on_delete=models.SET_NULL)
    ticket_code             = models.CharField(max_length=8, blank=True, null=True, default='')
    message_stream          = models.ForeignKey(MessageStream, blank=True, null=True, on_delete=models.SET_NULL) # Messaging integration
    details                 = models.TextField(blank=True, null=True)
...
class Ticket(Object):
    ...
    sla        = models.ForeignKey(ServiceLevelAgreement, blank=True, null=True, on_delete=models.SET_NULL)
    ...
    message    = models.ForeignKey(Message, blank=True, null=True, on_delete=models.SET_NULL) # Messaging integration
    details    = models.TextField(blank=True, null=True)
...
class TicketRecord(UpdateRecord):
    "Update for a Ticket"
    message = models.ForeignKey(Message, blank=True, null=True) # Messaging integration
