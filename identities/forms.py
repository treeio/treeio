...
class ContactTypeForm(forms.ModelForm):
    ...
    def __init__(self, user, *args, **kwargs):        
        ...
        self.fields['fields'].queryset = Object.filter_permitted(user, ContactField.objects.all())
    ...
    def clean_name(self):
        ...
        if instance and not instance.id:
            ...
            if ContactType.objects.filter(slug=slug).exists():
                raise forms.ValidationError(_("Contact Type with such name already exists."))
        return name
...
class ContactForm(forms.Form):
    ...
    name = forms.CharField(max_length=256, widget=forms.TextInput(attrs={'size':'50'}))
    instance = None
    files = {}
    ...
    def _get_form_field(self, field, value=None):
        ...
        if field.field_type == 'text':
            ...
            form_field = forms.EmailField(label=field.label, widget=forms.TextInput(attrs={'size':'30'}))
    ...
    def __init__(self, user=None, contact_type=None, *args, **kwargs):
        ...
        self.fields['parent'] = forms.ModelChoiceField(label='Parent', queryset=[], required=False)
        self.fields['parent'].queryset = Object.filter_permitted(user, Contact.objects)
        self.fields['parent'].widget.attrs.update({'class': 'autocomplete', 
                                                   'callback': reverse('identities_ajax_contact_lookup')})
        self.fields['parent'].widget.attrs.update({'popuplink': reverse('identities_contact_add')})
        ...
        if self.instance:
            ...
            if 'related_user' in self.fields:
                self.fields['related_user'].initial = self.instance.related_user_id
...
class SettingsForm(forms.Form):
    ...
    default_contact_type = forms.ModelChoiceField(label=_('Default Contact Type'), queryset=[])
