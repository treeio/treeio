...
class IdentitiesHandlersTest(TestCase):
    ...
    def test_get_fields_list(self):
        ...
        response = self.client.get(path=reverse('api_identities_types'), **self.authentication_headers)
        self.assertEquals(response.status_code, 200)

    def test_get_field(self):
        response = self.client.get(path=reverse('api_identities_fields', kwargs={'object_ptr': self.field.id}), **self.authentication_headers)
        self.assertEquals(response.status_code, 200)

    def test_update_field(self):
        updates = {'name': 'Api_update', 'required' : True, 'label': "api label", 'field_type': 'text'}
    ...
    def test_get_type(self):
        response = self.client.get(path=reverse('api_identities_types', kwargs={'object_ptr': self.contact_type.id}), **self.authentication_headers)
        self.assertEquals(response.status_code, 200)

    def test_update_type(self):
        updates = {'name': 'Api update', 'details' : 'Api test details', 'fields': [self.field.id]}
    ...
    def test_get_contacts_list(self):
        ...
        response = self.client.get(path=reverse('api_identities_contacts'), **self.authentication_headers)
        self.assertEquals(response.status_code, 200)

    def test_get_contact(self):
        response = self.client.get(path=reverse('api_identities_contacts', kwargs={'object_ptr': self.contact.id}), **self.authentication_headers)
        self.assertEquals(response.status_code, 200)
    ...
    def test_update_contact(self):
        ...
        self.assertEquals(data['contactvalue_set'][0]['value'], updates['Test___0'])

    def test_update_contact_with_picture(self):
        # create field
        updates = {'name': 'picture', 'required' : True, 'label': "Picture", 'field_type': 'picture'}
        ...
        #update contact type
        updates = {'name': 'Api type', 'details' : 'Api test details', 'fields': [field_id, self.field.id]}
        response = self.client.put(path=reverse('api_identities_types', kwargs={'object_ptr': self.contact_type.id}),
                                   content_type=self.content_type,  data=json.dumps(updates), **self.authentication_headers)
        self.assertEquals(response.status_code, 200)
        ...
        for value in data['contactvalue_set']:
            if value['name'] == 'Test':
                ...
                self.assertTrue(value['value'].endswith(updates['picture___0']['name']))
